<template>
  <header class="bg-brand-header text-brand-text-dark p-4 flex justify-between items-center sticky top-0 z-10 shadow-md">
    <div class="flex items-center space-x-2">
       <svg class="h-8 w-8 text-brand-primary-light" fill="currentColor" viewBox="0 0 20 20"> <path fill-rule="evenodd" d="M10.496 2.132a1 1 0 00-.992 0l-7 4A1 1 0 003 8v7a1 1 0 100 2h14a1 1 0 100-2V8a1 1 0 00-.504-.868l-7-4zM10 9a1 1 0 100-2 1 1 0 000 2zm-2 4a1 1 0 11-2 0 1 1 0 012 0zm4 0a1 1 0 11-2 0 1 1 0 012 0z" clip-rule="evenodd" />
       </svg>
      <h1 class="text-xl font-semibold">Gerenciador de Projetos</h1>
    </div>
    ...
    <div class="relative">
       ...
       <div v-if="isSearchOpen" class="absolute right-0 mt-2 w-64 md:w-96 bg-white rounded-md shadow-lg p-2 z-20">
           <input
             type="text"
             v-model="searchTerm"
           ...
           />
           <ul v-if="searchTerm.length < 3 && recentSearches.length > 0" class="mt-2 text-sm text-gray-600">
               <li class="font-semibold mb-1">Recentes:</li>
               <li v-for="(search, index) in recentSearches" :key="index" class="p-1 hover:bg-gray-100 cursor-pointer flex justify-between items-center">
                   <span>{{ search }}</span>
                   <BaseButton @click.stop="removeRecentSearch(search)" class="text-red-500 text-xs">✕</BaseButton>
                </li>
            </ul>
            </div>
    </div>
